...
for _ in range(epochs):
    ...
    # 每个epoch之后计算一次验证集的准确率
    val_data, val_label = lfw.load_lfwcrop_data_batch(val_file)
    val_label = to_categorical(val_label, total_classes)
    val_pred = model.predict(val_data)
    correct_val_pred =K.equal(K.argmax(val_pred, 1), K.argmax(val_label, 1))
    val_acc = K.mean(K.cast(correct_val_pred, "float32"))
    val_acc = K.eval(val_acc)

    np.random.shuffle(train_file)
    if ((_+1)%10) == 0:
        model.save("denseid_model_"+str(_).zfill(4)+".h5")
